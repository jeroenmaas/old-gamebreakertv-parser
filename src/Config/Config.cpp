...
bool ConfigFile::SetSource(const char *file, bool ignorecase)
{
	...
	if(file != 0)
	{
		...
		if(!f)
		{
			sLog.Error("ConfigFile", "Could not open %s.", file);
			return false;
		}
...
parse:
			...
			if(in_block)
			{
				...
				if(in_multiline_quote)
				{
					...
					{
						sLog.Error("ConfigFile", "Quote without variable.");
						return false;
					}
			...
			else
			{
				...
				if(offset != string::npos)
				{
					...
					{
						sLog.Error("ConfigFile", "Block without name.");
						return false;
					}
			...
			{
				sLog.Error("ConfigFile", "Exception in config parsing.");
				return false;
			}
		...
		if(in_block)
		{
			sLog.Error("ConfigFile", "Unterminated block.");
			return false;
		}

		if(in_multiline_comment)
		{
			sLog.Error("ConfigFile", "Unterminated comment.");
			return false;
		}

		if(in_multiline_quote)
		{
			sLog.Error("ConfigFile", "Unterminated quote.");
			return false;
		}
